...
class Exp_Informer(Exp_Basic):
    ...
    # 训练集的训练
    def train(self, setting,info_dict,run_name_dir_ckp,run_ex_dir):
    ...
    # 测试集测试
    def test(self, setting,info_dict,run_ex_dir):
        ...
        # 取出每个预测数组的最后一个值，因为根据滑动窗口来看，最后一个值才是那个滑动窗口的预测值
        # print(type(preds.tolist()))
        # print("--------->>>>>>--------------")
        # print(type(trues.tolist()))
        # print("--------->>>>>>--------------")
    ...
    # 预测未来
    def predict(self, setting,run_name_dir_ckp, run_ex_dir,load=False):
        ...
        pl = preds[0].tolist()
        pl = [i[0] for i in pl]
        dfpl = pd.DataFrame(pl)
        filename = "result.csv"
        if os.path.exists(filename):
            dfpl_old = pd.read_csv(filename)
            dfpl = pd.concat([dfpl_old,dfpl],axis=1)
            dfpl.to_csv(filename, sep=',', encoding='utf-8',index=False)
        else:
            dfpl.to_csv(filename, sep=',', encoding='utf-8',index=False)
        '''
        # result save
